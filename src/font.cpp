...
char32_t PETSCIItoUnicode(uint8_t petscii) {
    ...
    static char32_t petsciiMapping[256]
        = {
              ...
              /* 0xDC, */ 0x0001FB8C, // (!) braille 4 vertical dots (bad)
...
static void fillPetCatMap(std::unordered_map<std::string, char32_t>& petcatMap) {
    ...
    for (int n = 0; n < 256; ++n) {
        ...
        petcatMap["{" + dec + "}"]  = PETSCIItoUnicode(n);
    }
    ...
    petcatMap["{CBM-T}"] = PETSCIItoUnicode(0xe3); // upper one eights bar
    petcatMap["{$a3}"]   = PETSCIItoUnicode(0xe3);

    petcatMap["SHIFT-^"] = PETSCIItoUnicode(0x7e); // pi
    petcatMap["$ff"]     = PETSCIItoUnicode(0x7e);

    petcatMap["CBM-*"] = PETSCIItoUnicode(0x7f); // upper right triangle
    petcatMap["$df"]   = PETSCIItoUnicode(0x7f);

    petcatMap["SHIFT-@"] = PETSCIItoUnicode(0xba); // lower right blocks
    petcatMap["$fa"]     = PETSCIItoUnicode(0xba);
    ...
    petcatMap["{flon}"] = PETSCIItoUnicode(130);
    petcatMap["{flof}"] = PETSCIItoUnicode(132);
...
char32_t parseNextPetcat(const char*& str) {
    ...
    if (ch >= 'A' && ch <= 'Z') {
        ...
        return PETSCIItoUnicode(ch + 0xc0 - 0x40);
    }
