...
const bool OsWindowsConsole::isKeyPressed(uint32_t index, bool withShift, bool withAlt, bool withCtrl) const {
    ...
    if (withShift) { pressed &= ((GetAsyncKeyState(int(VK_LSHIFT)) & 0x8000) != 0 || (GetAsyncKeyState(int(VK_RSHIFT)) & 0x8000) != 0); }
    if (withAlt) { pressed &= ((GetAsyncKeyState(int(VK_LMENU)) & 0x8000) != 0); }
    if (withCtrl) { pressed &= ((GetAsyncKeyState(int(VK_LCONTROL)) & 0x8000) != 0 || (GetAsyncKeyState(int(VK_RCONTROL)) & 0x8000) != 0); }
    return pressed;
}
