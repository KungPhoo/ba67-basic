...
#define ASSERT(a)                                                   \
    {                                                               \
        if (!(a)) { throw Basic::Error(Basic::ErrorId::INTERNAL); } \
    }
...
void cmdRENUMBER(Basic* basic, const std::vector<Basic::Value>& values) {
    ...
    if (values.size() > 6) { milestone = int(basic->valueToInt(values[6])); }
    ...
    for (auto& [oldLine, code] : listing)
    {
        if (oldLine >= 0 && oldLine >= startFromLine)
        {
            ...
            if (oldLine > newLineNumber && milestone != 0 && (oldLine % milestone) == 0) { newLineNumber = oldLine; }
...
Basic::Basic(Os& os, SoundSystem* ss) {
    ...
    functions.insert({{"ABS", [&](Basic* basic, const std::vector<Basic::Value>& args) -> Basic::Value { nargs(args, 1); return fabs(basic->valueToDouble(args[0])); }},
    ...
    printUtf8String("\n");
    printUtf8String(
        std::string(centerx, ' ') + std::string(" ****    BA67 BASIC ") + "V" + version() + "    ****\n");
...
std::vector<Basic::Token> Basic::tokenize(ProgramCounter* pProgramCounter) {
    ...
    while (*pc != '\0')
    {
        ...
        if (parseKeyword(pc, &str))
        {
            ...
            if (str.length() == 3 && str == "REM")
            {  // REM ans SYS are special
...
Basic::Value Basic::evaluateDefFnCall(Basic::FunctionDefinition& fn, const std::vector<Basic::Value>& arguments) {
    ...
    for (size_t i = 0; i < fn.parameters.size(); ++i)
    {
        ...
        if (c == '$') { valType = TokenType::STRING; }
        else if (c == '%') { valType = TokenType::INTEGER; }
...
static std::string printUsing(const std::string& format, double value) {
    ...
    {
        signPos = p;
        if (forcePlusSign) { throw Basic::Error(Basic::ErrorId::SYNTAX); }
    }
...
void Basic::handleIFTHEN(std::vector<Token>& tokens) {
    ...
    {  // skip rest of the line CBM BASIC style. Dartmouth would evaluate the next command.
        programCounter().line++;
        programCounter().position = 0;
    }
}
...
void Basic::handleREAD(std::vector<Token>& tokens) {
    for (size_t itk = 1; itk < tokens.size(); ++itk)
    {
        ...
        if (tk.type == TokenType::IDENTIFIER)
        {
            ...
            {
                case '$':
                    if (!valueIsString(val)) { throw Error(ErrorId::TYPE_MISMATCH); }
                    break;
                default:
                case '%':
                case '#':
