...
#define ASSERT(a) {if(!(a)) {throw Basic::Error(Basic::ErrorId::INTERNAL);}}
...
void cmdCHAR(Basic* basic, const std::vector<Basic::Value>& values) {
    ...
    if (values.size() < 5) { throw Basic::Error(Basic::ErrorId::ARGUMENT_COUNT); }
    int ipara = 0;
    for (size_t i = 0; i < values.size(); ++i) {
        ...
        case 1: x = int(Basic::valueToInt(v));          break;
        case 2: y = int(Basic::valueToInt(v));          break;
        case 3: text = Basic::valueToString(v);         break;
        case 4: inverse = Basic::valueToInt(v) != 0;      break;
...
void cmdCHDIR(Basic* basic, const std::vector<Basic::Value>& values) {
    ...
    dir = basic->findFirstFileNameWildcard(dir);
...
void cmdFIND(Basic* basic, const std::vector<Basic::Value>& values) {
    ...
    for (auto& ln : basic->currentModule().listing) {
        ...
        for (size_t s = sline.length(); s < 3; ++s) { sline += ' '; }
        sline += ' ';
        sline += ln.second;
        sline += '\n';
...
void cmdRENUMBER(Basic* basic, const std::vector<Basic::Value>& values) {
    ...
    if (values.size() > 0) { newstart = int(basic->valueToInt(values[0])); }
    if (values.size() > 2) { step = int(basic->valueToInt(values[2])); }
    if (values.size() > 4) { startFromLine = int(basic->valueToInt(values[4])); }
...
void cmdPLAY(Basic* basic, const std::vector<Basic::Value>& values) {
    if (values.size() != 1) { throw Basic::Error(Basic::ErrorId::ARGUMENT_COUNT); }
    if (!basic->valueIsString(values[0])) { throw Basic::Error(Basic::ErrorId::TYPE_MISMATCH); }
...
void cmdSOUND(Basic* basic, const std::vector<Basic::Value>& values) {
    if (values.size() != 3) { throw Basic::Error(Basic::ErrorId::ARGUMENT_COUNT); }
    if (!basic->valueIsString(values[2])) { throw Basic::Error(Basic::ErrorId::TYPE_MISMATCH); }
...
void cmdCATALOG(Basic* basic, const std::vector<Basic::Value>& values) {
    if (values.size() != 0) { throw Basic::Error(Basic::ErrorId::ARGUMENT_COUNT); }
    ...
    basic->printUtf8String(basic->os->getCurrentDirectory());
    basic->printUtf8String("###\n");
    std::string str;
    for (size_t i = 0; i < files.size(); ++i) {
        auto& f = files[i].name;
...
Basic::Value fktINSTR(Basic* basic, const std::vector<Basic::Value>& args) {
    ...
    if (startOffset < 1) { throw Basic::Error(Basic::ErrorId::ILLEGAL_QUANTITY); }
...
Basic::Value fktMID$(Basic* basic, const std::vector<Basic::Value>& args) {
    ...
    if (start < 1) { throw Basic::Error(Basic::ErrorId::ILLEGAL_QUANTITY); }
    --start; // base 0

    size_t length = ~size_t(0);
    if (args.size() == 5) { length = basic->valueToInt(args[4]); }
...
void Basic::Array::dim(const ArrayIndex& ai) {
    ...
    try {
        for (size_t n = 1; n < ai.index.size(); ++n) {
            if (ai.index[n] != 0) { elems *= (1 + ai.index[n]); }
            if (elems < old) { throw Error(ErrorId::ILLEGAL_QUANTITY); }
            old = elems;
        }
...
Basic::Value& Basic::Array::at(const Basic::ArrayIndex& ix) {
    ...
    for (size_t n = 0; n < bounds.index.size(); ++n) {
        ...
        if (blk == 0 && ixn > 0) { throw Error(ErrorId::BAD_SUBSCRIPT); } // DIM a(5): a(1,1) = 1
        i += ixn * blk;
        blk *= bounds.index[n];
    }
...
Basic::Basic(Os& os, SoundSystem* ss) {
    ...
    printUtf8String(
        std::string(centerx, ' ')
        + std::string(" ****    BA67 BASIC ")
        + "V" + version()
        + "    ****\n"
    );

    std::string strmem = " " + valueToString(int64_t(os.getFreeMemoryInBytes()))
        + std::string(" BASIC BYTES FREE");
...
inline double Basic::valueToDouble(const Value& v) {
    ...
    if (auto s = std::get_if<std::string>(&v)) {
        ...
        double d = 0;
        if (parseDouble(str, &d) && *str == '\0') { return d; }
    }
    throw Error(ErrorId::TYPE_MISMATCH);
}
...
inline int64_t Basic::valueToInt(const Value& v) {
    ...
    if (auto s = std::get_if<std::string>(&v)) {
        const char* str = s->c_str();
        int64_t i = 0;
        if (parseInt(str, &i) && *str == '\0') { return i; }
    }
    throw Error(ErrorId::TYPE_MISMATCH);
}

bool Basic::valueIsOperator(const Value& v) {
    if (auto i = std::get_if<Basic::Operator>(&v)) return true;
    return false;
}

bool Basic::valueIsString(const Value& v) {
    if (auto s = std::get_if<std::string>(&v)) { return true; }
    return false;
}
...
inline bool Basic::parseDouble(const char*& str, double* number) {
    ...
    if (number) { *number = d; }
    if (end > str) { str = end; return true; }
    return false;
}
...
inline bool Basic::parseInt(const char*& str, int64_t* number) {
    ...
    if (endInt == str) { return false; }

    // might be a double?
    if (base == 10) {
        const char* endDbl = str;
        if (!parseDouble(endDbl)) { throw Error(ErrorId::SYNTAX); }
        if (endDbl > endInt) { return false; }
    }
    str = endInt;
    return true;
}
...
inline bool Basic::parseOperator(const char*& str, std::string* op) {
    ...
    while (*str == '=' || *str == '+' || *str == '-' || *str == '*' || *str == '/'
        || *str == '<' || *str == '>'
        || *str == '^' || *str == ';') {
        if (*str == '-' && str > start) { break; } // a=-2
        ++str;
    }
    if (str > start && op) { *op = std::string(start, str); return true; }
    return false;
}
...
inline bool Basic::parseIdentifier(const char*& str, std::string* identifier) {
    ...
    if ((*str >= 'A' && *str <= 'Z') || (*str >= 'a' && *str <= 'z')) {
        ...
        if (identifier) { *identifier = std::string(str, pend); }
        str = pend;
        return true;
    }
    return false;
...
std::vector<Basic::Token> Basic::tokenize(ProgramCounter* pProgramCounter) {
    ...
    for (size_t i = 1; i + 1 < tokens.size(); ++i) {
        ...
        if (toki.type == TokenType::OPERATOR
            && (
            tok_1.type == TokenType::OPERATOR
            || tok_1.type == TokenType::KEYWORD
            || tok_1.type == TokenType::UNARY_OPERATOR
            || (tok_1.type == TokenType::PARENTHESIS && tok_1.value == "(")
            )
            ) {
            toki.type = TokenType::UNARY_OPERATOR;
        }
...
inline int Basic::precedence(const std::string& op) {
    ...
    if (op == ";") { return __LINE__ - line0; } // ; is string concatenation operator
    if (op == "AND" || op == "OR") { return __LINE__ - line0; }
    if (op == "<" || op == ">" || op == "<>") { return __LINE__ - line0; }
    if (op == "=") { return __LINE__ - line0; }
    if (op == "+" || op == "-") { return __LINE__ - line0; }
    if (op == "*" || op == "/") { return __LINE__ - line0; }
    if (op == "^") { return __LINE__ - line0; }
    if (op[0] == 'u' || op == "NOT") { return __LINE__ - line0; } // unaray operator
    return 0;
}
...
Basic::Value Basic::evaluateDefFnCall(Basic::FunctionDefinition& fn, const std::vector<Basic::Value>& arguments) {
    ...
    for (size_t i = 0; i < fn.parameters.size(); ++i) {
        ...
        if (c == '$') { valType = TokenType::STRING; } else if (c == '%') { valType = TokenType::INTEGER; }
...
std::vector<Basic::Value> Basic::evaluateExpression(const std::vector<Token>& tokens, size_t start, size_t* ptrEnd) {
    ...
    auto applyOpSS = [](const std::string& a, const std::string& b, const std::string& op)->Value {
        if (op == "+") return a + b;
        if (op == "=") return a == b ? int64_t(-1) : int64_t(0);
        if (op == ">") return a > b ? int64_t(-1) : int64_t(0);
        if (op == "<") return a < b ? int64_t(-1) : int64_t(0);
        if (op == ">=") return a >= b ? int64_t(-1) : int64_t(0);
        if (op == "<=") return a <= b ? int64_t(-1) : int64_t(0);
    ...
    auto applyOpDD = [](double a, double b, const std::string& op)->Value {
        ...
        if (op == "AND") return int64_t(a) & int64_t(b);
        if (op == "OR")  return int64_t(a) | int64_t(b);
        if (op == "NOT") return ~int64_t(b);
        throw Error(ErrorId::SYNTAX);
        };
    auto applyOpII = [](int64_t a, int64_t b, const std::string& op)->Value {
        ...
        if (op == "-") return a - b;
        if (op == "*") return a * b;
        if (op == "/") return double(a) / double(b);
        if (op == "=") return a == b ? int64_t(-1) : int64_t(0);
        if (op == ">") return a > b ? int64_t(-1) : int64_t(0);
        if (op == "<") return a < b ? int64_t(-1) : int64_t(0);
        if (op == ">=") return a >= b ? int64_t(-1) : int64_t(0);
        if (op == "<=") return a <= b ? int64_t(-1) : int64_t(0);
        if (op == "<>") return a != b ? int64_t(-1) : int64_t(0);
        if (op == "^") return int64_t(pow(a, b));
        if (op == "AND") return int64_t(a) & int(b);
        if (op == "OR")  return int64_t(a) | int(b);
        if (op == "NOT") return ~int64_t(b);
        throw Error(ErrorId::SYNTAX);
        };
    auto applyOpDI = [](double a, int64_t b, const std::string& op)->Value {
        ...
        if (op == "*") return a * double(b);
        if (op == "/") return a / double(b);
        if (op == "=") return a == double(b) ? int64_t(-1) : int64_t(0);
        if (op == ">") return  a > b ? int64_t(-1) : int64_t(0);
        if (op == "<") return  a <  double(b) ? int64_t(-1) : int64_t(0);
        if (op == ">=") return a >= double(b) ? int64_t(-1) : int64_t(0);
        if (op == "<=") return a <= double(b) ? int64_t(-1) : int64_t(0);
    ...
    auto applyOpID = [](int64_t a, double b, const std::string& op)->Value {
        ...
        if (op == "*") return double(a) * b;
        if (op == "/") return double(a) / b;
        if (op == "=") return double(a) == b ? int64_t(-1) : int64_t(0);
        if (op == ">") return  double(a) > b ? int64_t(-1) : int64_t(0);
        if (op == "<") return  double(a) < b ? int64_t(-1) : int64_t(0);
        if (op == ">=") return double(a) >= b ? int64_t(-1) : int64_t(0);
        if (op == "<=") return double(a) <= b ? int64_t(-1) : int64_t(0);
    ...
    auto applyOp = [&]() -> void {
        ...
        if (da && db) { values.push_back(applyOpDD(*da, *db, op)); return; }
        const int64_t* ia = get_if<int64_t>(&a);
        const int64_t* ib = get_if<int64_t>(&b);
        if (ia && ib) { values.push_back(applyOpII(*ia, *ib, op)); return; }
        if (da && ib) { values.push_back(applyOpDI(*da, *ib, op)); return; }
        if (ia && db) { values.push_back(applyOpID(*ia, *db, op)); return; }

        const std::string* sa = get_if<std::string>(&a);
        const std::string* sb = get_if<std::string>(&b);
        if (sa && sb) { values.push_back(applyOpSS(*sa, *sb, op)); return; }
        if (sa) { values.push_back(applyOpSS(*sa, valueToString(b), op)); return; }
        if (sb) { values.push_back(applyOpSS(valueToString(a), *sb, op)); return; }
    ...
    for (size_t i = start; i < tokens.size(); i++) {
        ...
        if (toki.type == TokenType::NUMBER) {
            ...
            if (pval != nullptr) {
                ...
                if (end < tokens.size() && tokens[end].value != ")") { throw Error(ErrorId::SYNTAX); }
                if (end > i) {
                    i = end;
                    if (ptrEnd != nullptr) {
                        *ptrEnd = i;
            ...
            if (modit == modules.end()) { throw Error(ErrorId::UNDEFD_MODULE); }
            ++i;
            size_t end = i;
...
Basic::ArrayIndex Basic::indexFromValues(const std::vector<Basic::Value>& vals) {
    ...
    size_t i = 0;
    for (auto& v : vals) {
        if (valueIsOperator(v)) { continue; }
        int64_t iv = valueToInt(v);
        if (iv < 0) { throw Error(ErrorId::BAD_SUBSCRIPT); }
        ai.index[i++] = iv;
    }
    return ai;
}
...
inline void Basic::handleLET(const std::vector<Token>& tokens) {
    ...
    if (pval != nullptr) {
        ...
        if (values.size() != 1) { throw Error(ErrorId::SYNTAX); }

        switch (valuePostfix(tokens[ivarname])) {
        case '%': *pval = valueToInt(values[0]); break;
        case '$': *pval = values[0]; break;
        default:
        case '#': *pval = valueToDouble(values[0]); break;
        }
...
void Basic::handleRUN(const std::vector<Token>& tokens) {
    ...
    if (tokens.size() > 2) {
        ...
        if (runFrom < 0) { throw Error(ErrorId::UNDEFD_STATEMENT); }
    }
...
static std::string printUsing(const std::string& format, std::string value) {
    ...
    if (fieldWidth == 0) { throw Basic::Error(Basic::ErrorId::SYNTAX); }
    ...
    if (hasEqual) {
        if (hasRight) { throw Basic::Error(Basic::ErrorId::SYNTAX); }
...
static std::string printUsing(const std::string& format, double value) {
    ...
    if (p != std::string::npos) { signPos = p; forcePlusSign = true; }
    p = format.find('-');
    if (p != std::string::npos) {
        signPos = p; if (forcePlusSign) { throw Basic::Error(Basic::ErrorId::SYNTAX); }
    }
    ...
    if (firstHash == std::string::npos) { throw Basic::Error(Basic::ErrorId::SYNTAX); }
    ...
    for (size_t itk = 1; itk < tokens.size(); ++itk) {
        ...
        if (tk.type == TokenType::IDENTIFIER) {
            ...
            if (key.printable) {
                ...
                switch (key.code) {
                case uint32_t(Os::KeyConstant::CRSR_LEFT):  Unicode::appendAsUtf8(str, 0x009d); break; // L    157 9d
                case uint32_t(Os::KeyConstant::CRSR_RIGHT): Unicode::appendAsUtf8(str, 0x001d); break; // R     29 1d
                case uint32_t(Os::KeyConstant::CRSR_UP):    Unicode::appendAsUtf8(str, 0x0091); break; // U    145 91
                case uint32_t(Os::KeyConstant::CRSR_DOWN):  Unicode::appendAsUtf8(str, 0x0011); break; // D     17 11
                case uint32_t(Os::KeyConstant::F1):         Unicode::appendAsUtf8(str, 0x0085); break; // F1   133 85
                case uint32_t(Os::KeyConstant::F2):         Unicode::appendAsUtf8(str, 0x0086); break;
                case uint32_t(Os::KeyConstant::F3):         Unicode::appendAsUtf8(str, 0x0087); break;
                case uint32_t(Os::KeyConstant::F4):         Unicode::appendAsUtf8(str, 0x0088); break;
                case uint32_t(Os::KeyConstant::F5):         Unicode::appendAsUtf8(str, 0x0089); break;
                case uint32_t(Os::KeyConstant::F6):         Unicode::appendAsUtf8(str, 0x008a); break;
                case uint32_t(Os::KeyConstant::F7):         Unicode::appendAsUtf8(str, 0x008b); break;
                case uint32_t(Os::KeyConstant::F8):         Unicode::appendAsUtf8(str, 0x008c); break; // F8   140 8c
            ...
            case '%': *pval = valueToInt(str); break;
            case '$': *pval = str; break;
            default: case '#': *pval = valueToDouble(str); break;
            }
        }
    }
}
        ...
        if (itk == 1 && tk.type == TokenType::STRING) {
            ...
            while (badinput) {
                try {
                    ...
                    switch (valuePostfix(tk)) {
                    case '%': *pval = valueToInt(s); break;
                    case '$': *pval = s; break;
                    default: case '#': *pval = valueToDouble(s); break;
                    }
                    badinput = false;
...
inline void Basic::handleDIM(const std::vector<Token>& tokens) {
    ...
    for (size_t i = 1; i < tokens.size(); ++i) {
        if (tokens[i].type != TokenType::IDENTIFIER) { throw Error(ErrorId::SYNTAX); }
        std::string varName = tokens[i].value;
        ++i;
        if (tokens[i].type != TokenType::PARENTHESIS || tokens[i].value != "(") { throw Error(ErrorId::SYNTAX); }
...
void Basic::handleLIST(const std::vector<Token>& tokens) {
    ...
    for (auto& ln : currentModule().listing) {
        if (ln.first < from) { continue; }
        if (ln.first > to) { break; }
...
void Basic::handleKEY(const std::vector<Token>& tokens) {
    ...
    if (tokens.size() < 4) { throw Basic::Error(Basic::ErrorId::ARGUMENT_COUNT); }

    size_t k = strToInt(tokens[1].value) - 1;
    if (k >= keyShortcuts.size()) { throw Basic::Error(Basic::ErrorId::ILLEGAL_QUANTITY); }
...
char Basic::valuePostfix(const Token& t)const {
    if (t.value.ends_with('$')) { return '$'; }
    if (t.value.ends_with('%')) { return '%'; }
    if (t.type == TokenType::INTEGER) { return '%'; }
    if (t.type == TokenType::NUMBER) { return '#'; }
    if (t.type == TokenType::STRING) { return '$'; }
    return '#';
}
...
Basic::Value* Basic::findLeftValue(Module& module, const std::vector<Token>& tokens, size_t start, size_t* endPtr) {
    ...
    if (tokens.size() > i + 2 && tokens[i + 1].type == TokenType::PARENTHESIS && tokens[i + 1].value == "(") {
        ...
        size_t end = i;
        auto args = evaluateExpression(tokens, i + 2, &end);
        if (tokens[end].value != ")") { throw Error(ErrorId::SYNTAX); }
        i = end + 1; // after brace
    ...
    if (tokens[i].type == TokenType::IDENTIFIER) {
        ...
        if (varit == variables.end()) {
            ...
            case '%': variables[tokens[i].value] = 0LL; break;
            case '$': variables[tokens[i].value] = ""; break;
            default:
            case '#': variables[tokens[i].value] = 0.0; break;
            }
...
void Basic::handleDEFFN(std::vector<Token>& tokens) {
    ...
    if (tokens.empty()) { throw Error(ErrorId::SYNTAX); }
...
void Basic::handleFOR(const std::vector<Token>& tokens) {
    if (tokens.size() < 6 || tokens[2].value != "=") return;
...
void Basic::handleNEXT(const std::vector<Token>& tokens) {
    ...
    if ((loop.step > 0 && loopVar <= loop.end) || (loop.step < 0 && loopVar >= loop.end)) {
        ...
        if (modl.programCounter.line == modl.listing.end()) { throw Error(ErrorId::INTERNAL); }
...
Basic::Value Basic::readNextData() {
    ...
    for (;;) {
        ...
        if (tokens.empty()) {
            ++cm.readDataPosition.line; cm.readDataPosition.position = 0;
            cm.readDataIndex = 0;
...
void Basic::handleREAD(std::vector<Token>& tokens) {
    for (size_t itk = 1; itk < tokens.size(); ++itk) {
        ...
        if (tk.type == TokenType::IDENTIFIER) {
            ...
            switch (valuePostfix(tk)) {
            case '$': if (!valueIsString(val)) { throw Error(ErrorId::TYPE_MISMATCH); } break;
            default: case '%': case '#': if (valueIsString(val)) { throw Error(ErrorId::TYPE_MISMATCH); } break;
            }
            *pval = val;
        }
    }
...
std::string Basic::inputLine(bool allowVertical) {
    ...
    auto typeString = [&](const std::string& s) {
        ...
        for (auto& c : valueToString(vals[0])) { os->putToKeyboardBuffer(c); }
        };
    ...
    for (;;) {
        ...
        if (!key.printable) {
            ...
            case uint32_t(Os::KeyConstant::F3):  typeString(keyShortcuts[2]);  continue;
            case uint32_t(Os::KeyConstant::F4):  typeString(keyShortcuts[3]);  continue;
            case uint32_t(Os::KeyConstant::F5):  typeString(keyShortcuts[4]);  continue;
            case uint32_t(Os::KeyConstant::F6):  typeString(keyShortcuts[5]);  continue;
            case uint32_t(Os::KeyConstant::F7):  typeString(keyShortcuts[6]);  continue;
            case uint32_t(Os::KeyConstant::F8):  typeString(keyShortcuts[7]);  continue;
            case uint32_t(Os::KeyConstant::F9):  typeString(keyShortcuts[8]);  continue;
            case uint32_t(Os::KeyConstant::F10): typeString(keyShortcuts[9]); continue;
            case uint32_t(Os::KeyConstant::F11): typeString(keyShortcuts[10]); continue;
            case uint32_t(Os::KeyConstant::F12): typeString(keyShortcuts[11]); continue;
            case uint32_t(Os::KeyConstant::BACKSPACE): os->screen.backspaceChar(); continue;
            case uint32_t(Os::KeyConstant::DEL): os->screen.deleteChar(); continue;
...
inline void Basic::dumpVariables() {
    ...
    if (currentModule().variables.empty() && currentModule().arrays.empty()) { return; }
...
Basic::ParseStatus Basic::parseInput(const char* pline) {
    ...
    try {
        ...
        if (parseInt(pc, &n)) {
            ...
            if (n < 0) { throw Error(ErrorId::SYNTAX); }
        ...
        int lastTraceLine = -1;
        for (;;) {
            if (programCounter().line == currentListing().end()) { break; }
...
void Basic::waitForKeypress() {
    for (;;) {
        ...
        if (os->keyboardBufferHasData()) { break; }
        os->delay(100);
        os->updateKeyboardBuffer();
    }
}
...
bool Basic::loadProgram(std::string filenameUtf8) {
    ...
    if (buff == nullptr) { fclose(file); return false; }
